...
def analyze_transactions(transactions):
    ...
    for tx in transactions:
        ...
        # --- 2. 규칙 기반 모델 적용 ---
        if tx_time:
            # 규칙 1: '늦은 밤(22시~02시)'에 '배달 음식' 또는 '택시' 이용 (15달러 이상)
            ...
            if is_late_night and is_delivery_or_taxi and amount > 15:
                event_desc = f"{tx_time.strftime('%m/%d %H:%M')}에 {tx.get('name')}에서 ${amount:,.2f} 결제 (늦은 밤 소비)"
                emotional_spending_events.append(event_desc)

            # 규칙 2: '주말'에 '쇼핑(SHOPS)'에서 $50 이상 결제
            is_weekend = tx_time.weekday() >= 5  # 토(5), 일(6)
            is_shopping = "SHOPS" in category
            if is_weekend and is_shopping and amount >= 50:
                event_desc = f"{tx_time.strftime('%m/%d')} 주말에 {tx.get('name')}에서 ${amount:,.2f} 결제 (주말 충동구매 의심)"
                emotional_spending_events.append(event_desc)
